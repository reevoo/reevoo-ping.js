...
window.reevooPing = {
  ...
  sendEventsWhen: (eventNames) => {
    window.reevooPing.clearEvents();

    window.reevooPing.events = eventNames.map(eventCatalogue.get);
    window.reevooPing.events.forEach(event => { event.attach(event.trigger); });
  },

  // Clear the event list, safely detach()'ing each event before clearing.
  clearEvents: () => {
    window.reevooPing.events.forEach(event => { event.detach(); });
    window.reevooPing.events = [];
  },
