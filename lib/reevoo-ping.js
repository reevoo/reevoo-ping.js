...
import eventSetCatalogue from './event_sets';
...
const reevooPing = {
  ...
  sendEventSet: function sendEventsWhen(eventSetName) {
    const eventSet = eventSetCatalogue[eventSetName];
    if (!eventSet) { error(`Event Set ${eventSetName} not found.`); }
    reevooPing.sendEventsWhen(eventSet);
  },
};
