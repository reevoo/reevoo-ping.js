...
export default function event(opts) {
  if (!opts.name) {
    error(`Event defined without a name!\n${opts}`);
  }

  if (!opts.attach) {
    error(`Event ${opts.name} defined without an attach() method!`);
  }

  if (!opts.trigger) {
    error(`Event ${opts.name} defined without a trigger() method!`);
  }

  if (!opts.detach) {
    error(`Event ${opts.name} defined without a detach() method!`);
  }
